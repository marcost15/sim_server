...
elseif($_REQUEST['set']=='disable' ||$_REQUEST['set']=='enable'){
	...
	if($_REQUEST[sim]) {
		...
		if($row=$db->fetch_array($query)){
			if($_REQUEST['set']=='disable') {
				$db->query("update sim set dev_disable=1 where sim_name='$_REQUEST[sim]'");
				$db->query("insert into logs set sim_name='$_REQUEST[sim]', log='disabled manually by api'");
			}
			else {
				$db->query("update sim set dev_disable=0 where sim_name='$_REQUEST[sim]'");
				$db->query("insert into logs set sim_name='$_REQUEST[sim]', log='enabled manually by api'");
			}
