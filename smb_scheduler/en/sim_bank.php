		...
		if($ErrMsg!="")
			...
			if($banktype=='SMB128') {
				for($i=1;$i<=128;$i++){
					...
					$sql.="('$name".sprintf("%03d",$i)."','$team_id', '$name', '$_POST[imei_mode]','$imei', '$remain_time', '$time_unit','$remain_time','$_POST[no_ring_limit]','$_POST[no_answer_limit]','$_POST[short_call_limit]','$_POST[short_time]', '$_POST[no_ring_disable]','$_POST[no_answer_disable]','$_POST[short_call_disable]','$_POST[count_limit]','$_POST[count_limit]','$_POST[no_connected_limit]','$_POST[no_connected_limit]','$_POST[month_limit_time]','$_POST[month_limit_time]','$_POST[month_reset_day]', now())";
					if($i<128) $sql .= ",";
				}
			}
			else {
