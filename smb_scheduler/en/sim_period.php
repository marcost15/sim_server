...
if($_REQUEST['chkAll0'] || $sim_name=="ALL") {
	$sim_name="ALL";
	$where="where 1";
}else {
	$where="where sim_name in ($sim_name)";
}
...
if($action=='modify'){
	...
	for($i=0;$i<=6;$i++)
		for($j=1;$j<=9;$j++){
			...
			$sl=$_REQUEST['remain_sms'.$i.$j];
			if(!isset($l) || $l==='') $l=-1;
			if(!isset($c) || $c==='') $c=-1;
			if(!isset($sl) || $sl==='') $sl=-1;
			if($s && $e && ($l>=0 || $c>=0 || $sl>=0) && $s!=':' && $e!=':' )
				$sql.="$i)$s-$e,$l,$c,$sl;";
	}
	$sql.="' $where";
	$db->query($sql);
	$query=$db->query("select sim_name,period_limit from sim $where");
	while($row=$db->fetch_array($query)) {
		$db->query("insert into logs set sim_name='$row[sim_name]', log='change period limit'");
		$send[]=my_pack($row, SIM_PERIOD);
	}
	sendto_xchanged($send);
	WriteSuccessMsg("<br><li>Save Success</li>","all_sim.php");
}
